...
app.post('/nf-followup-data', function(request, response) {
    console.log("Posting data")
    // Convert to CSV
    DATA_CSV = JSON2CSV(request.body);
    ...
    filename = "nf_followup/" + ID_DATE + "_nf_followup" + ".csv";
    saveDropbox(DATA_CSV, filename);
    response.end();
});

app.post('/bs-followup-data', function(request, response) {
    console.log("Posting data")
    // Convert to CSV
    DATA_CSV = JSON2CSV(request.body);
    ...
    filename = "bs_followup/" + ID_DATE + "_bs_followup" + ".csv";
    saveDropbox(DATA_CSV, filename);
    response.end();
});
